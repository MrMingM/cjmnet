...
def settings(options):
    ...
    integer_positive = ('tile_size', 'actions_per_view', 'hidden', 'epochs')
    if any(not isinstance(value[key], int) or value[key] < 1 for key in integer_positive):
        raise ValueError('tile_size/actions_per_view/hidden/epochs must be positive integers')
...
def contract(options, frontend, digest):
    ...
    sources = sorted((ROOT/'local_fusion_utility').glob('*.py'))
    specification = dict(specification)
    specification['schema'] = 1
    specification['method'] = 'local_fusion_post_nms_utility'
...
def validate_contract(specification):
    ...
    if specification.get('method') != 'local_fusion_post_nms_utility':
