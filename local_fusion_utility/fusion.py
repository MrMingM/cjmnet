...
def _pool(value, grid, maximum=False):
    return (F.adaptive_max_pool2d if maximum else F.adaptive_avg_pool2d)(value, grid)


def _feature_descriptors(level, baseline, weights, grid):
    peers = level[1:]
    ...
    return [_pool(value, grid) for value in maps]


def _prediction_descriptors(local, full, grid):
    ...
    return [
        _pool(peer_cls, grid), _pool(peer_cls, grid, True),
        _pool(full_cls, grid), _pool(full_cls, grid, True),
        _pool(cls_delta, grid), _pool(cls_delta, grid, True),
        _pool(reg_delta.mean(1, keepdim=True), grid),
        _pool(reg_delta.amax(1, keepdim=True), grid, True),
        _pool(peer_reg.abs().mean(1, keepdim=True), grid),
        _pool(full_reg.abs().mean(1, keepdim=True), grid),
    ]
    ...
    if peer_count:
        ...
        for scale in (0, 1):
            descriptors.extend(_feature_descriptors(
                levels[scale], baseline[scale], original_weights[scale], grid))
        descriptors.extend(_prediction_descriptors(local, full, grid))
