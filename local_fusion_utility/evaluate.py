    ...
    with torch.no_grad(), (output/'frames.jsonl').open('w', encoding='utf-8') as stream:
        for batch in loader:
            ...
            for method in methods:
                if method == 'baseline':
                    continue
                source = 'utility' if method.startswith('utility') else method
                ...
                action = select_actions(score_cache[source], thresholds[source], allow_keep)
...
def main():
    ...
    thresholds = {name: float(value['threshold']) for name, value in calibration['methods'].items()}
