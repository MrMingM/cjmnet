...
def set_uncertainty_override(hypes, opt):
    fusion_args = hypes['model']['args']['where2comm_fusion']
    communication = fusion_args['communication']
    if opt.use_uncertainty != 'keep':
        communication['use_uncertainty'] = opt.use_uncertainty == 'true'
    if opt.uncertainty_alpha is not None:
        communication['use_uncertainty'] = True
        communication['uncertainty_alpha'] = opt.uncertainty_alpha
        print('Override uncertainty_alpha to %.4f' % opt.uncertainty_alpha)
    if opt.comm_threshold is not None:
        communication['threshold'] = opt.comm_threshold
        print('Override communication threshold to %.4f' % opt.comm_threshold)


def set_weather_reliability_override(hypes, opt):
    fusion_args = hypes['model']['args']['where2comm_fusion']
    weather_cfg = fusion_args.setdefault('weather_reliability', {})
    if opt.weather_reliability == 'off':
        weather_cfg['enable'] = False
    elif opt.weather_reliability == 'density':
        weather_cfg['enable'] = True
        weather_cfg['apply_to'] = 'comm'
        weather_cfg['density_enable'] = True
        weather_cfg['isolation_enable'] = False
        weather_cfg['alpha_density'] = 0.2
        weather_cfg['beta_isolation'] = 0.0
        weather_cfg['reliability_floor'] = 0.9
        weather_cfg['empty_cell_reliability'] = 1.0
    elif opt.weather_reliability == 'density_isolation':
        weather_cfg['enable'] = True
        weather_cfg['apply_to'] = 'comm'
        weather_cfg['density_enable'] = True
        # Conservative setting after initial tests: keep the CLI name for
        # compatibility, but disable isolation so communication is not crushed.
        weather_cfg['isolation_enable'] = False
        weather_cfg['alpha_density'] = 0.2
        weather_cfg['beta_isolation'] = 0.0
        weather_cfg['reliability_floor'] = 0.9
