...
class Communication(nn.Module):
    def __init__(self, args):
        ...
        self.use_uncertainty = args.get('use_uncertainty', False)
        self.uncertainty_weight = args.get('uncertainty_weight', 1.0)
        self.uncertainty_alpha = args.get('uncertainty_alpha', self.uncertainty_weight)
        self.uncertainty_eps = args.get('uncertainty_eps', 1e-6)
    ...
    def forward(self, batch_confidence_maps, B):
        ...
        communication_rates = []
        for b in range(B):
            confidence_maps, _ = batch_confidence_maps[b].sigmoid().max(dim=1, keepdim=True)
            if self.use_uncertainty:
                prob = confidence_maps.clamp(min=self.uncertainty_eps,
                                             max=1.0 - self.uncertainty_eps)
                entropy = -(prob * torch.log(prob) +
                            (1.0 - prob) * torch.log(1.0 - prob)) / np.log(2.0)
                reliability = torch.clamp(1.0 - self.uncertainty_alpha * entropy,
                                          min=0.0, max=1.0)
                ori_communication_maps = confidence_maps * reliability
            else:
                ori_communication_maps = confidence_maps
            if self.smooth:
