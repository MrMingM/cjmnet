...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        # GT index assigned to every positive anchor.  ``-1`` denotes an
        # anchor without a GT assignment.  This is training metadata used by
        # target-aware distillation; it is never consumed during inference.
        pos_gt_index = np.full(
            (*feature_map_shape, self.anchor_num), -1, dtype=np.int64)
        ...
        valid_gt_indices = np.flatnonzero(masks == 1)
        pos_gt_index[index_x, index_y, index_z] = \
            valid_gt_indices[id_pos_gt]
    ...
    def collate_batch(label_batch_list):
        ...
        targets = []
        pos_gt_index = []
        has_pos_gt_index = all(
            'pos_gt_index' in labels for labels in label_batch_list)
        ...
        for i in range(len(label_batch_list)):
            ...
            if has_pos_gt_index:
                pos_gt_index.append(label_batch_list[i]['pos_gt_index'])
        ...
        if has_pos_gt_index:
            target_batch['pos_gt_index'] = torch.from_numpy(
                np.array(pos_gt_index))
        return target_batch
