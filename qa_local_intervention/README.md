...
Stage-3C 的 full 最终分数重放采用绝对误差上限 1e-6、相对误差 0。
这是针对实测 4.47034836e-7 差值的浮点比较修正，前提是最终框编号、匹配和 FP 数量完全一致；
框坐标容差、源码、输入与目标状态检查不变。每帧日志与 replay_comparison 保存实际最大差值，超过上限仍停止。
